...
def sync_lists(trello, trello_board_id, board_id):
    ...
    with connection.cursor(cursor_factory=RealDictCursor) as cur:
        for board_list in board_lists:
            ...
            sync_card(trello, trello_list_id, db_list.get('id'))


# Cards on a list
def sync_card(trello, trello_list_id, list_id):
...
def sync_card_members(trello_card_id, user_ids):
    with connection.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        if not data:
            ...
            existing_user_id = [row.get('user_id') for row in data]
            must_remove_ids = list(set(existing_user_id).difference(set(user_ids))) # noqa
            for remove_id in must_remove_ids:
                cur.execute(queries.DELETE_CARD_MEMBER, (card_id, remove_id))
                connection.commit()
            must_add_ids = list(set(user_ids).difference(set(existing_user_id)))
